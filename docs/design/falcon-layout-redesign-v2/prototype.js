...
const pages = [
  { id: "dashboard", file: "01-dashboard.html", label: "仪表盘", group: "工作台", badge: "HOME", width: "", family: "overview" },
  { id: "collector-home", file: "02-collector-home.html", label: "采集首页", group: "采集", badge: "TODAY", width: "", family: "overview" },
  { id: "collector-runs", file: "03-collector-runs.html", label: "任务队列", group: "采集", badge: "RUNS", width: "wide", family: "data" },
  { id: "collector-create", file: "04-collector-create.html", label: "任务创建", group: "采集", badge: "NEW", width: "", family: "config" },
  { id: "collector-accounts", file: "05-collector-accounts.html", label: "账号管理", group: "采集", badge: "PROFILE", width: "wide", family: "data" },
  { id: "collector-environment", file: "06-collector-environment.html", label: "环境自检", group: "采集", badge: "CHECK", width: "wide", family: "ops" },
  { id: "collector-run-detail", file: "07-collector-run-detail.html", label: "任务详情", group: "采集", badge: "DETAIL", width: "wide", family: "inspector" },
  { id: "collector-sample-preview", file: "08-collector-sample-preview.html", label: "样本预览", group: "采集", badge: "MEDIA", width: "wide", family: "inspector" },
  { id: "keywords", file: "09-keywords.html", label: "关键词池", group: "基础", badge: "PLAN", width: "", family: "config" },
  { id: "report", file: "10-report.html", label: "日报", group: "基础", badge: "DOC", width: "doc", family: "document" },
  { id: "analysis-home", file: "11-analysis-home.html", label: "分析首页", group: "分析", badge: "INSIGHT", width: "", family: "overview" },
  { id: "analysis-samples", file: "12-analysis-samples.html", label: "分析样本", group: "分析", badge: "SAMPLES", width: "wide", family: "data" },
  { id: "review", file: "13-review.html", label: "人工复核", group: "分析", badge: "REVIEW", width: "wide", family: "inspector" },
  { id: "execution-home", file: "14-execution-home.html", label: "执行首页", group: "执行", badge: "DRAFT", width: "", family: "overview" },
  { id: "tasks", file: "15-tasks.html", label: "触达任务", group: "执行", badge: "TASKS", width: "wide", family: "data" },
];

const byId = Object.fromEntries(pages.map((page) => [page.id, page]));

const runs = [
  ["xiaohongshu-20260524-090257", "小红书 / default", "账号增长", "completed", "已完成", "2026-05-24 09:02", "5 / 23 / 34", "查看"],
  ["xiaohongshu-20260524-101840", "小红书 / creator", "小红书封面", "manual", "需人工", "2026-05-24 10:18", "8 / 18 / 12", "继续采集"],
  ["xiaohongshu-20260524-103312", "小红书 / backup", "内容脚本", "running", "运行中", "2026-05-24 10:33", "3 / 9 / 8", "运行中"],
  ["xiaohongshu-20260524-104500", "小红书 / default", "增长工具", "queued", "待启动", "2026-05-24 10:45", "0 / 0 / 0", "启动采集"],
  ["xiaohongshu-20260524-082044", "小红书 / default", "运营复盘", "failed", "失败", "2026-05-24 08:20", "2 / 4 / 6", "重新运行"],
  ["xiaohongshu-20260523-224112", "小红书 / creator", "素材管理", "completed", "已完成", "2026-05-23 22:41", "12 / 40 / 56", "查看"],
...
const samples = [
  ["65", "账号增长怎么做才不像模板？", "账号增长", "小鹿同学", "0.91", "增长路径 / 可复用流程", "预览"],
  ["66", "封面字太乱，怎么一眼高级", "小红书封面", "麦麦", "0.84", "封面层级 / 标题留白", "预览"],
  ["67", "素材管理风格一直崩", "素材管理", "花园计划", "0.81", "身份照可信度", "预览"],
  ["68", "增长工具是不是都长一样", "增长工具", "Rita", "0.78", "差异化模板", "预览"],
  ["69", "评论区求提示词集合", "内容脚本", "海盐", "0.76", "提示词包需求", "预览"],
  ["70", "为什么生成封面不适合小红书", "封面设计", "风铃", "0.73", "平台尺寸和裁切", "预览"],
  ["71", "账号增长商用会不会侵权", "账号增长", "K先生", "0.70", "版权与肖像", "预览"],
];
...
function button(text, kind = "ghost") {
  return `<a class="button${cls(kind)}" href="#" data-label="${text}"><span>${text}</span></a>`;
}
...
function panel(title, subtitle, body, side = "") {
  return `<section class="panel" data-title="${title}">
...
function renderShell(pageId, content) {
  ...
  const groupCounts = Object.fromEntries(groups.map((group) => [group, pages.filter((page) => page.group === group).length]));
  const nav = groups.map((group) => `<section class="nav-section${group === active.group ? " current" : ""}">
  ...
  return `<div class="prototype-shell">
    <header class="topbar">
      <a class="brand" href="index.html"><span class="brand-mark">F</span><div><strong>Falcon</strong><small>prototype v2</small></div></a>
      <div class="topbar-context">
        <span>${active.group}</span>
        <strong>${active.label}</strong>
      </div>
      <div class="system-strip" aria-label="Local system status">
        <span>LOCAL</span>
        <span>GPT-5.5</span>
    ...
    <div class="workspace-frame">
      <aside class="sidebar">
        <div class="rail-summary">
          ...
          <strong>${pages.length} views</strong>
        </div>
        <div class="nav-section-wrap">${nav}</div>
        <div class="rail-footer">
          <span>${active.family}</span>
          <strong>${groupCounts[active.group]} / ${active.group}</strong>
        </div>
      </aside>
      <main class="main"><div class="page${cls(active.width)} family-${active.family}" data-view="${pageId}">${content}</div></main>
    </div>
  </div>`;
}
...
const templates = {
    ...
    return pageHeader(meta, [button("初始化数据库"), button("整理采集计划", "primary")]) +
      ...
      `<section class="layout-grid">
        ${panel("今日待办", "只保留可行动项，入口不展示完整长表。", `<div class="stack">
          ...
          <div class="thin-card"><h3>3 个 queued run 可启动</h3><p>default profile 空闲，可以串行启动 账号增长 / 增长工具任务。</p><div class="inline-actions">${button("进入任务队列", "primary")}</div></div>
        ...
        <div class="stack">
          ...
          ${panel("最近日报", "文档入口只展示摘要和打开动作。", `<div class="callout">2026-05-24 日报已生成，包含 账号增长、封面设计、素材管理三个主题的高价值样本和可写选题。</div><div class="inline-actions" style="margin-top:10px">${button("阅读日报", "primary")}${button("重新生成")}</div>`)}
        </div>
      </section>`;
  },
    ...
    return pageHeader(meta, [button("任务队列"), button("创建任务", "primary")]) +
      ...
      `<section class="layout-grid">
        ${panel("待处理焦点", "可立即处理的 run 放在首页，不把完整任务表塞进来。", `<div class="stack">
          ...
          <div class="thin-card"><h3>1 个失败任务可重新入队</h3><p>运营复盘 run 保存了错误截图，建议重新运行新任务。</p><div class="inline-actions">${button("重新运行")}${button("查看证据")}</div></div>
    ...
    return pageHeader(meta, [button("批量启动"), button("创建任务", "primary")]) +
      `<section class="panel tight">
        <div class="filters">
          ...
          <div class="field medium"><label>关键词</label><input value="账号增长"></div>
          ${button("筛选", "primary")}
        </div>
    ...
    return pageHeader(meta, [button("返回队列"), button("加入队列", "primary")]) +
      `<section class="layout-grid reverse">
        ...
        ${panel("任务参数", "平台和 Profile 先定，关键词支持标签化拆分。", `<div class="form-grid">
          ...
          <div class="field"><label>任务标签</label><input value="增长复盘素材"></div>
          <div class="field full"><label>关键词组</label><div class="chips"><span class="chip">账号增长</span><span class="chip">小红书封面</span><span class="chip">素材管理</span></div><input style="margin-top:8px" placeholder="输入关键词，按 Enter 或逗号添加"></div>
        </div>`)}
      </section>`;
  },
  ...
  "collector-run-detail"() {
    ...
    return pageHeader(meta, [button("返回任务队列"), button("重新运行")]) +
      ...
      panel("当前状态与操作", "单一状态区，避免页面上反复展示同一套状态。", `<div class="facts columns-3">
        ...
        <div class="fact"><span>关键词</span><strong>账号增长</strong></div>
      ...
      `<section class="preview-grid">
        ...
        <div class="stack">
          ${panel("正文与热评", "聚焦内容判断，不展示本地文件路径。", `<div class="stack">
            <div class="thin-card"><h3>正文</h3><p>第一次做 账号增长，最怕生成出来像模板照。这里记录真实感、五官稳定和背景统一的提示词组合。</p></div>
            <div class="thin-card"><h3>热评 01</h3><p>想要一套适合运营复盘的提示词，最好能直接复制。</p></div>
  ...
  keywords() {
    ...
    const rows = ["账号增长", "小红书封面", "素材管理", "增长工具", "内容脚本", "封面设计"].map((word, index) => `<tr><td>内容运营</td><td>${word}</td><td>${index % 2 ? "转化素材" : "需求采样"}</td><td>${index + 3}</td><td>${10 + index * 2}</td></tr>`).join("");
    return pageHeader(meta, [button("导入 CSV"), button("生成默认池", "primary")]) +
      `<section class="layout-grid reverse">
        ${panel("生成配置", "按内容定宽，不把输入框做成大横条。", `<div class="form-grid">
          ...
          <div class="field medium"><label>主题</label><input value="内容运营"></div>
  ...
  report() {
    ...
    return pageHeader(meta, [button("返回总览"), button("导出 Markdown")]) +
      ...
      `<section class="panel doc-body">
        <h2>今日结论</h2>
        <p>账号增长相关样本中，用户更关心“真实感”和“可直接复制的提示词”，而不是泛泛的生成效果展示。</p>
        <p>封面设计类样本集中在标题层级、留白和平台裁切问题，适合拆成教程型内容。</p>
        <h2>建议选题</h2>
        <p>1. 账号增长不像模板照的 5 个提示词修正点。</p>
        <p>2. 小红书封面标题为什么显乱：3 层信息结构。</p>
        <p>3. 素材管理风格如何避免“塑料感”。</p>
        <h2>风险与备注</h2>
    ...
    return pageHeader(meta, [button("分析样本"), button("送入分析队列", "primary")]) +
      ...
      `<section class="layout-grid">
        ...
        <div class="stack">
          ...
          ${panel("选题洞察", "只保留可写方向摘要。", `<div class="stack"><div class="thin-card"><h3>增长路径</h3><p>用户追求不像模板、眼神自然、可商用。</p></div><div class="thin-card"><h3>封面层级</h3><p>标题和元素拥挤，是高频痛点。</p></div></div>`)}
        </div>
      </section>`;
  },
  ...
  "analysis-samples"() {
    ...
    return pageHeader(meta, [button("批量送复核"), button("导出样本")]) +
      `<section class="panel tight">
        <div class="filters">
          <div class="field compact"><label>关键词</label><select><option>全部</option><option>账号增长</option></select></div>
    ...
    return pageHeader(meta, [button("上一条"), button("保存并下一条", "primary")]) +
      `<section class="review-workbench">
        ...
        ${panel("复核表单", "表单动作独立，避免每行都拉宽。", `<div class="stack">
          <div class="thin-card"><h3>当前样本</h3><p>账号增长怎么做才不像模板？</p><small>痛点：增长路径 / 可复用流程</small></div>
    ...
    return pageHeader(meta, [button("触达任务"), button("生成草稿", "primary")]) +
      ...
      `<section class="layout-grid">
        ${panel("待确认草稿", "首页只展示优先队列摘要。", `<div class="stack">
          <div class="task-card"><div class="task-head"><strong>[高] 账号增长真实感教程评论</strong>${status("待确认", "waiting")}</div><p class="muted">评论草稿：可以先从眼神、肤色和背景三个变量排查...</p></div>
          ...
          <div class="task-card"><div class="task-head"><strong>[中] 素材管理私信草稿</strong>${status("素材待补", "queued")}</div><p class="muted">需要确认是否生成配图示例。</p></div>
        </div>`)}
        <div class="stack">
  ...
  tasks() {
    ...
    const taskRows = [
      ["T-1024", "账号增长真实感教程评论", "评论", "高", "pending", "待确认"],
      ["T-1025", "封面层级选题发布预览", "发帖", "高", "pending", "待确认"],
      ["T-1026", "素材管理私信回复", "私信", "中", "copied", "已复制"],
    ...
    return pageHeader(meta, [button("批量更新"), button("打开执行预览", "primary")]) +
      `<section class="layout-grid reverse">
        ${panel("任务筛选", "按状态和优先级管理，不占大面积。", `<div class="form-grid">
          ...
          <div class="field medium"><label>搜索</label><input value="账号增长"></div>
