...
class WebAppTest(unittest.TestCase):
    ...
    def test_analysis_queue_lists_edits_details_and_deletes_without_collection_queue(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('data-analysis-execute-form', queue.text)
            self.assertIn('data-submitting-label="正在执行任务"', queue.text)
            self.assertIn('class="analysis-queue-pending-feedback"', queue.text)
            self.assertIn("已接收执行请求，Falcon 正在匹配帖子和评论。", queue.text)
            self.assertIn("event.preventDefault()", queue.text)
            self.assertIn("fetch(form.action", queue.text)
            self.assertIn("window.location.href = response.url", queue.text)
